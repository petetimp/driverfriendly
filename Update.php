    ...
    <div>
        ...
        ?>
            <div id="add_submitted">
                ...
                <p>You have now updated your listing.  Please wait 4-6 hours for your listing to be approved.
                   If your listing is approved, you will be rewarded .25 reward points.  10 reward points get
            ...
            <div id="add_listing">
                ...
                <div>
                    <span class="label4" id="label4">Edit the Information Below</span>
                </div>
                <?php
					if ($stmt = $mysqli2->prepare("SELECT title, city, highway, address, start_date, end_date, event_description, event_type, created, last_updated, website, created_by FROM roadwork_areas WHERE listing_id=?")) {
						
						// Bind a variable to the parameter as a string. 
						$stmt->bind_param("i", $_GET['listing_id']);
						...
						$stmt->bind_result($title, $city, $highway, $address, $startDate, $endDate, $eventDescription, $eventType, $created, $lastUpdated, $website, $createdBy);
   	   	        ...
   	   	        ?>
                        <form id="add_form" onSubmit="return false">
                            <table id="add_page_table">
                                <tbody>
                                    <tr>
                                        ...
                                        <td><input type="text" id="textbox12" placeholder="Title" class="textbox1" value="<?php echo $title ?>"></td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php
											    if($city==1){
													echo "<script>
														document.getElementById('add_select').selectedIndex=1;
													</script>";
												}else{
													echo "<script>
														document.getElementById('add_select').selectedIndex=2;
													</script>";
												}
                                            ?>
                                        </td>                            
                                    ...
                                    <tr>
                                        <td>Start Date*</td>
                                        <td><input type="text" id="textbox2" placeholder="Start Date" class="textbox1" value="<?php echo $startDate ?>"></td>  
                                    </tr>
                                    <tr>
                                        <td>End Date</td>
                                        <td><input type="text" id="textbox2a" placeholder="End Date" class="textbox1" value="<?php echo $endDate ?>"></td>  
                                    </tr>
                                    <tr>
                                        <td>Address of Event</td>
                                        <td><input type="text" id="textbox3" placeholder="Address" class="textbox1" value="<?php echo $address ?>"></td>
                                    </tr>
                                    <tr>
                                        <td>Road Affected*</td>
                                        <td><input type="text" id="textbox4" placeholder="Highway/Street*" class="textbox1" value="<?php echo $highway ?>"></td>
                                    </tr>
                                </tbody>
                            </table>
                            <label class="add_page_label">Provide a Website for More Information
                                <input type="text" id="textbox5" placeholder="Website for More Information" class="textbox1" value="<?php echo $website ?>">
                            ...
                            <?php
							    if($eventType==1){
								    echo "<script>
					   					     document.getElementById('event_select').selectedIndex=1;
										 </script>";
								}else if($eventType==2){
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=2;
										</script>";
								
								}else if($eventType==3){
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=3;
										</script>";
								
								}else if($eventType==4){
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=4;
										</script>";
								
								}else if($eventType==5){
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=5;
										</script>";
								
								}else if($eventType==6){
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=6;
										</script>";
								
								}else{
								    echo "<script>
										     document.getElementById('event_select').selectedIndex=7;
										</script>";
								}
								/*
								if($eventType==1){
									echo "<script>
											document.getElementById('checkbox1a').checked=true;
										 </script>";
								}else if($eventType==2){
									echo"<script>
											document.getElementById('checkbox3a').checked=true;
										</script>";
								}else{
								    echo"<script>
											document.getElementById('checkbox2a').checked=true;
										</script>"; 
								}*/
                             ?>
                            <div id="desc_e">Describe The Event Here*</div><br/>
                                <textarea class="textbox1" id="textarea1" placeholder="Briefly describe the event here*"><?php echo $eventDescription ?></textarea>
        
                            <span class="label5" id="label5">*Required</span>
                            <input type="submit" id="button4" value="Update" class="button1"/>
                        </form>
                </div>
        ...
        <?php
				}else{echo "<script>location.href='http://104.236.221.43/DriverFriendly/';</script>";}
            }else{
                echo "<div class='containerbox1' id='containerbox1'>
        ...
        <script type="text/javascript">
            ...
            function addingScreen(title, selected_option, startDate, endDate, address, highway, website, selected_event , description){
                ...
                <?php echo "var listing_id='". $_GET['listing_id'] ."';"; ?>
				listing_id=parseInt(listing_id);
                xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded;");
                xmlhttp.send("title=" + title + "&selected_option=" + selected_option + "&startDate=" + startDate + "&endDate=" + endDate + "&address=" + address + "&highway=" + highway + "&website=" + website + "&selected_event=" + selected_event + "&description=" + description + "&created_by=" + username + "&listing_id=" +listing_id+"&section=update");
